...
public interface CommentRepository extends CrudRepository<Comment, Long>,
    ...
    @Query("select c from Comment c where c.discussion.id = :discussionId and lower(c.text) like lower(concat('%', :text, '%')) order by c.date desc")
    List<Comment> findByTextPattern(@Param("text") String text, @Param("discussionId") Long discussionId);
    
    @Query("select c from Comment c where c.discussion.id = :discussionId and lower(c.author.username) like lower(concat('%', :username, '%')) order by c.date desc")
    List<Comment> findByAuthorUsername(@Param("username") String username, @Param("discussionId") Long discussionId);
    
    @Query("select c from Comment c where c.discussion.id = :discussionId order by c.date")
    List<Comment> findByDiscussionId(@Param("discussionId") Long discussionId);
    
    @Query("select c from Comment c where c.discussion.id = :discussionId order by c.rating")
    List<Comment> findByDiscussionIdOrderByRatingAsc(@Param("discussionId") Long discussionId);
    
    @Query("select c from Comment c where c.discussion.id = :discussionId order by c.rating desc")
